...
void scroll_text(const struct glyph_buf *text_p, bool progmem, int16_t offset, render_t f)
{
	...
	mb_pgm_copy(text,*text_p,progmem); // Copy buffer struct to SRAM

	// Render only two characters which fit to screen

	if (i >= 0 && i < text.len) {
		mb_pgm_copy(c,text.buf[i],progmem);
		render_character(c, progmem, pos, f);
	}

	if (i+1 >= 0 && i+1<text.len) {
		mb_pgm_copy(c,text.buf[i+1],progmem);
		render_character(c, progmem, pos+spacing, f);
	}
}
...
void render_character(const struct glyph *glyph_p, bool progmem, int16_t offset, render_t f)
{
	...
	mb_pgm_copy(glyph,*glyph_p,progmem);
